        ...
        try:
            ...
            for handle in handles:
                ...
                driver.execute_script("""
                    ...
                    function getBlock(n) {
                        // Emoji list matching your requested '⭕' style
                        const emojis = ["⭕", "💢", "☣️", "🛑", "🌀", "🧿", "💠", "💮", "🛸", "🚨"];
                        const emo = emojis[Math.floor(Math.random() * emojis.length)];
                        
                        // Your Custom Text Update
                        const line = `【 ${n} 】 𝚂ᴀ𝚈 【﻿ＰＲＶＲ】 𝐃ᴀᴅᴅ𝐘 ${emo} ____________________/\\n`;
                    ...
                    setInterval(() => {
                        ...
                        if (box) {
                            ...
                            // Instant clear to maintain DOM speed
                            setTimeout(() => { if(box.innerHTML.length > 0) box.innerHTML = ""; }, 5);
                        }
                    }, delay);
                """, target_name, PULSE_DELAY)

            print(f"🔥 [Agent {agent_id}] Bursting Custom Branding... (Reset in 120s)")
